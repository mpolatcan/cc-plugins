---
name: ccbell-nightly:configure
description: Configure ccbell-nightly sound notifications for different events
allowed-tools: ["Read", "Write", "AskUserQuestion", "Bash"]
---

# Configure ccbell-nightly Sound Notifications

Help the user configure sound notifications for Claude Code events.

## Available Sounds

ccbell-nightly uses bundled sounds by default for consistent cross-platform support:
  ...
  "questions": [
    {
      ...
      "options": [
        ...
        {"label": "Yes", "description": "Log all events to ~/.claude/ccbell-nightly.log for troubleshooting"}
      ]
    }
  ]
}
...
Create the configuration file at `~/.claude/ccbell-nightly.config.json`:
...
After writing the config, confirm to the user and offer to test the sounds with `/ccbell-nightly:test`.
...
Use `/ccbell-nightly:profile` to switch between saved profiles.
