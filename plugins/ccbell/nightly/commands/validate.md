---
name: ccbell-nightly:validate
description: Validate ccbell-nightly installation and configuration
allowed-tools: ["Read", "Bash"]
---

# Validate ccbell-nightly Installation

Run a comprehensive validation of the ccbell-nightly plugin installation and configuration.
...
```bash
echo "=== ccbell-nightly Validation ==="
echo ""
...
if [ -z "$CCBELL_PATH" ]; then
    ...
    echo ""
    echo "ccbell-nightly is NOT installed. To install, run:"
    echo "  /plugin marketplace add mpolatcan/cc-plugins"
    echo "  /plugin install ccbell-nightly"
    echo ""
    echo "For manual validation, ensure ccbell-nightly exists at:"
    echo "  ~/.claude/plugins/cache/<marketplace>/ccbell-nightly/<version>/"
    exit 1
fi
...
if [ -z "$LATEST_VERSION" ]; then
    ...
    echo "Please reinstall: /plugin install ccbell-nightly"
    exit 1
fi
...
echo "Checking/downloading ccbell binary (pre-release)..."
...
- Verify GitHub pre-releases are accessible: https://github.com/mpolatcan/ccbell/releases
- Ensure write permission to `$PLUGIN_ROOT/bin`
- Check that the pre-release tag exists for the configured version
